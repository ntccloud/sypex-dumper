		...
		<table cellspacing="5">
			<tr>
				...
				<legend>{$SXD->LNG['auth']}</legend>
					...
					<table cellspacing="3" id="hst" style="display:block;">
						<tr>
							<td width="90">{$SXD->LNG['my_host']}</td>
							<td width="100"><input type="text" name="host" style="width:90px;" value="{$_POST['host']}"></td>
							<td width="46" align="right">{$SXD->LNG['my_port']}</td>
							<td width="30"><input type="text" name="port" maxlength="5" style="width:40px;text-align:center;" value="{$_POST['port']}"></td>
						</tr>
						<tr>
							<td>Language:</td>
							<td style="padding-top:5px;"><select type="text" name="lang">{$SXD->lng_list}</select></td>
						</tr>
					</table>
				</fieldset>
				</td>
			...
			<tr class="buttons">
				<!--<td align="left"><input type="button" value="{$SXD->LNG['btn_details']}" onclick="var s = document.getElementById('hst').style; s.display = (s.display == 'block') ? 'none' : 'block';"></td>-->
				<td align="right"><input type="submit" value="{$SXD->LNG['btn_enter']}"></td>
			</tr>
		</table>
		</form>
	</div>
